...
struct Vector3 {
    ...
    [[nodiscard]] real mag() const noexcept;

    // Computes the square of the magnitude of the vector 
    // More efficient if all you are doing is comparing the magnitude of the vectors
    [[nodiscard]] real sqr_mag() const noexcept;

    // normalizes vector in place
    void norm() noexcept;

    // Subtracts two vectors
    [[nodiscard]] Vector3 subtract(const Vector3& other) const noexcept;

    // Adds two vectors
    [[nodiscard]] Vector3 add(const Vector3& other) const noexcept;

    // Scale vector by real number
    [[nodiscard]] Vector3 scale(const real& s) const noexcept;

    // Computes the vector product (elementwise multiplication), completely useless
    [[nodiscard]] Vector3 component_product(const Vector3& other) const noexcept;

    // Computes the dot product (sometimes called the Scalar Product)
    // a . b = |a| |b| cos theta
    [[nodiscard]] real dot_product(const Vector3& other) const noexcept;

    // Computes the cross product (sometimes called the Vector Product)
    // a x b = |a| |b| sin theta
    [[nodiscard]] Vector3 cross_product(const Vector3& other) const noexcept;

    // Computes the angle in radians between two vectors
    // theta = cos-1(a . b / |a| |b|)
    [[nodiscard]] real angle_between(const Vector3& other) const noexcept;

    // Gives us a Basis from vector a and b, with a being retained as one of the
    // orthogonal axes
    [[nodiscard]] static std::optional<Basis3> basis_from(Vector3& a, Vector3& b) noexcept;
